        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if KEY != 2 and not any(color_list[63:]):
                    ...
                    if not any(color_list[63:]) and any(color_list[:7]):
                        b = color_list.copy()
                        color_list.clear()
                        color_list = b[63:]
                        color_list.extend(b[:63])
                        b.clear()

                    for i in range(5):
                        for g, rect in enumerate(block_list[:7]):
                            if rect.collidepoint(randint(0, 420), 58):
                            # if randint(0, 1):

                                if color_list[g] == "":
                                    color_list[g] = colors_dict[randint(0, 5)]
